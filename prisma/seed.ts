...
async function main() {
  ...
  // Use upsert for categories to avoid duplicate key errors when re-running
  // the seed script. Category has a composite unique constraint on
  // (storeId, slug) so we upsert by that composite key.
  const categoryDefs = [
    {
      name: 'Electronics',
      slug: 'electronics',
  ...
  for (const def of categoryDefs) {
    // Prisma composite unique fields are exposed via a single object key
    // named by joining field names with an underscore (storeId_slug).
    // Use upsert so re-running the seed won't fail with P2002.
    const cat = await prisma.category.upsert({
      where: { storeId_slug: { storeId: demoStore.id, slug: def.slug } },
      update: {
        name: def.name,
  ...
  const brandDefs = [
    { name: 'TechPro', slug: 'techpro', description: 'Premium electronics brand', isPublished: true },
    { name: 'StyleCo', slug: 'styleco', description: 'Fashion and lifestyle brand', isPublished: true },
    { name: 'HomeEssentials', slug: 'home-essentials', description: 'Quality home products', isPublished: true },
  ];
  ...
  console.log(`✓ Created or updated ${brands.length} brands\n`);
  ...
  // Products: use upsert to avoid duplicate (storeId, slug) conflicts on re-seed
  const productDefs = [
    // Electronics - Featured Products
    {
      ...
      description:
        'Premium noise-cancelling wireless headphones with 30-hour battery life. Features advanced active noise cancellation, high-fidelity audio drivers, and comfortable over-ear design. Perfect for music lovers, travelers, and remote workers.',
      ...
      metaDescription:
        'Shop premium wireless headphones with 30-hour battery life and active noise cancellation. Free shipping over $50.',
    ...
    },
    {
      ...
      description:
        'Advanced fitness tracking smartwatch with heart rate monitor, GPS, and sleep tracking. Water-resistant up to 50m. Compatible with iOS and Android. Track your workouts, monitor health metrics, and stay connected on the go.',
    ...
    // Clothing - Multiple Products
    {
      ...
      description:
        '100% organic cotton t-shirt with comfortable fit. Pre-shrunk, breathable fabric. Available in multiple colors and sizes. Perfect for everyday wear.',
  ...
  for (const def of productDefs) {
    const p = await prisma.product.upsert({
      ...
      update: {
        ...
        inventoryQty: def.inventoryQty,
        lowStockThreshold: def.lowStockThreshold,
        inventoryStatus: def.inventoryStatus as any,
        weight: def.weight,
        length: def.length,
        width: def.width,
        height: def.height,
        categoryId: def.categoryIndex != null ? categories[def.categoryIndex].id : undefined,
        brandId: def.brandIndex != null ? brands[def.brandIndex].id : undefined,
        images: def.images,
      ...
      create: {
        ...
        categoryId: def.categoryIndex != null ? categories[def.categoryIndex].id : undefined,
        brandId: def.brandIndex != null ? brands[def.brandIndex].id : undefined,
        images: def.images || JSON.stringify([]),
        thumbnailUrl: def.thumbnailUrl,
        metaTitle: def.metaTitle,
        metaDescription: def.metaDescription,
        metaKeywords: def.metaKeywords || JSON.stringify([]),
        isPublished: def.isPublished,
        publishedAt: def.publishedAt,
        isFeatured: def.isFeatured,
      },
  ...
  console.log(`✓ Created or updated ${products.length} products with comprehensive details\n`);
  ...
  const emailTemplates = [] as any[];
  for (const def of emailDefs) {
    const et = await prisma.emailTemplate.upsert({
      where: { storeId_handle: { storeId: demoStore.id, handle: def.handle } },
      update: {
        name: def.name,
        subject: def.subject,
    ...
    emailTemplates.push(et);
  }

  console.log(`✓ Created or updated ${emailTemplates.length} email templates\n`);
  ...
  for (const def of customerDefs) {
    const c = await prisma.customer.upsert({
      where: { storeId_email: { storeId: demoStore.id, email: def.email } },
      update: {
        userId: def.userId,
        firstName: def.firstName,
        lastName: def.lastName,
        phone: def.phone,
        acceptsMarketing: def.acceptsMarketing,
        marketingOptInAt: def.marketingOptInAt,
        totalOrders: def.totalOrders,
        totalSpent: def.totalSpent,
        averageOrderValue: def.averageOrderValue,
        lastOrderAt: def.lastOrderAt,
      },
      create: {
        storeId: demoStore.id,
        userId: def.userId,
  ...
  console.log(`✓ Created or updated ${customers.length} customers\n`);
  ...
  const discounts = [] as any[];
  for (const def of discountDefs) {
    const d = await prisma.discount.upsert({
      where: { storeId_code: { storeId: demoStore.id, code: def.code } },
      update: {
        type: def.type as any,
        value: def.value,
        minimumPurchase: def.minimumPurchase,
        usageLimit: def.usageLimit,
        usageCount: def.usageCount,
        perCustomerLimit: def.perCustomerLimit,
  ...
  const flashSale = await prisma.flashSale.upsert({
    where: { id: 'black-friday-sale-demo' },
    update: {},
    create: {
      id: 'black-friday-sale-demo',
      storeId: demoStore.id,
  ...
  console.log(`✓ Created or updated ${discounts.length} discounts and flash sale: ${flashSale.name}\n`);
  ...
  // Orders: upsert by composite (storeId, orderNumber) to avoid duplicates
  const orderDefs = [
    {
    ...
    },
    {
      ...
      items: [
        { productIndex: 1, price: 299.99, quantity: 1, subtotal: 299.99, taxAmount: 24.75, totalAmount: 324.74 },
        { productIndex: 5, price: 29.99, quantity: 1, subtotal: 29.99, taxAmount: 2.47, totalAmount: 32.46 },
      ],
      payment: {
        amount: 372.19,
        currency: 'USD',
    ...
    },
    {
      ...
      items: [ { productIndex: 6, price: 79.99, quantity: 1, subtotal: 79.99, taxAmount: 6.6, totalAmount: 86.59 } ],
      payment: { amount: 92.58, currency: 'USD', status: 'PAID', method: 'CREDIT_CARD', gateway: 'STRIPE', gatewayPaymentId: 'pi_test_456789123' },
    },
  ];
  ...
  for (const def of orderDefs) {
    const o = await prisma.order.upsert({
      where: { storeId_orderNumber: { storeId: demoStore.id, orderNumber: def.orderNumber } },
      update: {}, // don't modify existing orders on re-seed
      create: {
        storeId: demoStore.id,
        customerId: customers[def.customerIndex].id,
        userId: def.userId,
        orderNumber: def.orderNumber,
        status: def.status as any,
        shippingAddressId: addresses[def.shippingAddressIndex].id,
        billingAddressId: addresses[def.billingAddressIndex].id,
        subtotal: def.subtotal,
        taxAmount: def.taxAmount,
        shippingAmount: def.shippingAmount,
        discountAmount: def.discountAmount,
        totalAmount: def.totalAmount,
        discountCode: def.discountCode,
        paymentMethod: def.paymentMethod as any,
        paymentGateway: def.paymentGateway as any,
        paymentStatus: def.paymentStatus as any,
        shippingMethod: def.shippingMethod,
        shippingStatus: def.shippingStatus as any,
        trackingNumber: def.trackingNumber,
        trackingUrl: def.trackingUrl,
        fulfilledAt: def.fulfilledAt,
        ipAddress: def.ipAddress,
        items: {
          create: def.items.map((it: any) => ({
            productId: products[it.productIndex].id,
            productName: products[it.productIndex].name,
            sku: products[it.productIndex].sku,
            image: products[it.productIndex].thumbnailUrl,
            price: it.price,
        ...
        payments: {
          create: {
            storeId: demoStore.id,
            amount: def.payment.amount,
            currency: def.payment.currency,
            status: def.payment.status as any,
            method: def.payment.method as any,
            gateway: def.payment.gateway as any,
            gatewayPaymentId: def.payment.gatewayPaymentId,
            gatewayCustomerId: def.payment.gatewayCustomerId,
          },
        },
      },
    });
  ...
  console.log(`✓ Created or updated ${orders.length} sample orders\n`);
  ...
  const newsletterDefs = [
    { email: customer.email, isSubscribed: true, source: 'checkout' },
    { email: 'jane.smith@example.com', isSubscribed: true, source: 'footer' },
    { email: 'marketing@example.com', isSubscribed: true, source: 'popup' },
  ];

  const newsletters = [] as any[];
  for (const def of newsletterDefs) {
    const n = await prisma.newsletter.upsert({
      where: { storeId_email: { storeId: demoStore.id, email: def.email } },
      update: { isSubscribed: def.isSubscribed, source: def.source, unsubscribedAt: null },
      create: { storeId: demoStore.id, email: def.email, isSubscribed: def.isSubscribed, source: def.source },
    });
    newsletters.push(n);
  }

  console.log(`✓ Created or updated ${newsletters.length} newsletter subscriptions\n`);
