...
- Q: Which authoritative session timeout values should be used (idle sliding timeout and absolute JWT expiration)? → A: 7-day idle sliding window and 30-day absolute JWT expiration (use FR-090 as authoritative).

- Q: How should "Remember Me" behave regarding cookie persistence vs server-side session idle timeout? → A: Persist the auth cookie for 30 days (Remember Me), but keep the server-side idle sliding timeout at 7 days; activity refreshes server session.

- Q: Which models should be treated as tenant-scoped and how should Prisma middleware handle tenant injection and opt-outs? → A: Treat the following as tenant-scoped models and enforce `storeId` injection via Prisma middleware: `product`, `variant`, `order`, `customer`, `category`, `brand`, `review`, `inventory`, `price`, `coupon`, `shipment`, `shippingRate`, `shippingZone`, `cart`, `cartItem`, `address`, `invoice`, `payment`, `refund`, `auditLog` (where applicable to store scope). Middleware behavior:
  - Auto-inject `storeId` for reads (`findMany`, `findFirst`, `findUnique` by adding `storeId` to the `where` clause), writes (`create`, `createMany` add `storeId` to `data`), and mutating ops (`update`, `updateMany`, `delete`) by adding `storeId` to `where` as appropriate.
  - Provide a documented opt-out escape hatch via an explicit flag (e.g., `PrismaContext.args.bypassTenant = true`) that must only be used by server-side admin tools or migration scripts and only when the calling code verifies SuperAdmin privileges. Opt-out usage MUST be logged and audited.
  - Fail fast: if middleware detects a tenant-scoped model access but cannot determine `storeId` from session/context, throw an explicit error to prevent accidental cross-tenant access.
  - Log middleware injections (debug/audit mode) and any opt-out events for traceability in production.
  - Administrative/cross-store models (e.g., `store`, `global_settings`, `admin_user`, `platform_audit`) are not auto-injected and must be accessed explicitly with appropriate privilege checks.
...
- **FR-090A**: The system MUST provide a login page at `/auth/login` with email input field (validated for email format), password input field (masked with show/hide toggle), "Remember Me" checkbox (persists auth cookie for 30 days; server-side idle sliding timeout remains 7 days), "Forgot Password" link, and "Sign In" button. Login page must be accessible without authentication and redirect authenticated users to their default dashboard.
...
- **FR-140**: System MUST support multi-store user access patterns: (1) **User-Store association**: User can be associated with multiple stores via join table (UserStore) with per-store role assignment (e.g., STORE_ADMIN at Store A, STAFF at Store B), (2) **Store switching UI**: Admin dashboard header displays current store name with dropdown to switch between associated stores; switching reloads dashboard with new store context without re-authentication, (3) **Role inheritance**: User's permissions determined by role assigned to specific store; no cross-store role inheritance (must explicitly assign role per store), (4) **Session context**: User session (FR-047) stores current active storeId; all API requests auto-filtered by session storeId via Prisma middleware for tenant isolation, (5) **Super Admin privileges**: SUPER_ADMIN role grants access to all stores without explicit association; store selector shows "All Stores" with search/filter capability, (6) **Audit logging**: Store switching events logged to audit trail with previous storeId, new storeId, and timestamp for security tracking.
 - **FR-141**: System MUST enforce tenant isolation via Prisma middleware (application layer):
   1. **Tenant-scoped models**: The middleware MUST treat the canonical set of tenant-scoped models as containing a `storeId` and enforce that `storeId` is applied to all queries. Canonical tenant-scoped models include: `product`, `variant`, `order`, `customer`, `category`, `brand`, `review`, `inventory`, `price`, `coupon`, `shipment`, `shippingRate`, `shippingZone`, `cart`, `cartItem`, `address`, `invoice`, `payment`, `refund`, and other store-scoped resources. Administrative/cross-store models (e.g., `store`, `global_settings`, `admin_user`, `platform_audit`) are explicitly excluded from auto-injection and must be accessed only by Super Admin guarded code paths.
   2. **Auto-injection rules**: Middleware MUST auto-inject `storeId` for read operations (`findMany`, `findFirst`, `findUnique`) by augmenting `where` clauses, for create operations (`create`, `createMany`) by adding `storeId` to `data`, and for mutating operations (`update`, `updateMany`, `delete`) by enforcing `where: { ...where, storeId }`. Middleware MUST also ensure `count` and aggregation queries respect tenant filters.
   3. **Opt-out / escape hatch**: Provide a documented opt-out mechanism (e.g., `{ bypassTenant: true }`) for internal migration/admin tooling. Opt-out usage MUST require Super Admin privilege checks and MUST be logged to the audit trail with reason, actor, and timestamp. The opt-out is intended only for server-side admin operations and maintenance scripts; it MUST NOT be exposed to tenant-level code.
   4. **Fail-fast on missing context**: If the middleware cannot determine a `storeId` from session/context when accessing tenant-scoped models, it MUST throw an error to prevent silent cross-tenant access. This error should provide actionable debugging guidance (e.g., "No storeId in session — Prisma tenant middleware blocked access to product.findMany").
   5. **Observability & auditing**: Middleware should emit logs (structured JSON) for each auto-injection event in debug/audit mode and record opt-out events in the audit log for post-incident review.
   6. **Raw SQL prohibition & RLS**: Raw SQL queries (`$queryRaw`, `$executeRaw`) remain prohibited in application code for Phase 1; PostgreSQL Row-Level Security (RLS) policies are planned for Phase 2 as a defense-in-depth measure. E2E tests must validate tenant isolation (cross-tenant attempts return 403/404 as appropriate).
