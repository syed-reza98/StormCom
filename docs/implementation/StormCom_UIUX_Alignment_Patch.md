# StormCom UI/UX Alignment Patch

Save the patch section below to a file named **`stormcom-uiux-alignment.patch`** and apply it from the repo root.

```bash
git fetch origin
git switch 001-multi-tenant-ecommerce
git checkout -b codex/uiux-spec-alignment
git apply --whitespace=fix stormcom-uiux-alignment.patch
git add -A && git commit -m "feat(specs): align with StormCom UI/UX Design System & Implementation Plan" && git push -u origin chore/uiux-spec-alignment
```
...
@@
 ## UI/UX Tasks
-- [ ] T009a — Establish Tailwind v4 config with semantic tokens
-- [ ] T009b — Create `globals.css` with light/dark variables
-- [ ] T009c — Wire Radix/shadcn primitives to tokens
-- [ ] T009d — Add Storybook and docs for components
-- [ ] T009e — Implement dark mode toggle and persistence
-- [ ] T009f — Add focus-visible and keyboard navigation patterns
-- [ ] T009g — Verify APCA contrast for interactive states
-- [ ] T009h — Hook tenant theme fields (primary/secondary/font) to CSS variables
