 ...
 * 
 * Shopify OAuth connection endpoint. Exchanges authorization code for access token
 * and saves encrypted configuration to database.
 * 
 * Request Body:
 * - code: string (Shopify authorization code from OAuth callback)
 * - shop: string (Shopify shop domain, e.g., 'test-store.myshopify.com')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { code, shop, state } = validation.data;
    ...
    const accessToken = await IntegrationService.exchangeShopifyCode(
      shop,
      code
    );

    // 5. Save configuration with encrypted credentials
    ...
    console.error('POST /api/integrations/shopify/connect error:', error);
